...
//--- over ---//
	...
	if(output_tgt=="")output_tgt=cwd+"/"+targetName+".tgt";
	string sequence_ = "";
	string sequence = "";
	char buf[BufLen];
...
// command to generate features 
	...
	string genACC_cmd = home_root + "/util/genACC.sh " + targetName + " "+temporary_root+"/"  + " "+home_root+"/";
// generate MAIN file
	{
...
// structural info
	...
	fout.close();	
	string fin_combine="";
	fin_combine = fin_combine + home_root + "/util/combine_file.sh "+temporary_root+"/"+targetName+".ss2 "+temporary_root+"/"+targetName+".ss8 "+temporary_root+"/"+targetName+".acc "+output_tgt+ " "+temporary_root;
...
// ---------- build ss3 and acc using SS8_To_SS3 and ACC_ReFormat ------------- //__2014_10_30__//
	string nnn_command;
	nnn_command="";
	nnn_command = nnn_command + home_root + "/util/SS8_To_SS3 "+temporary_root+"/"+targetName+".ss8 > "+temporary_root+"/"+targetName+".ss3";
	system(nnn_command.c_str());
	nnn_command="";
	nnn_command = nnn_command + home_root + "/util/ACC_ReFormat "+temporary_root+"/"+targetName+".acc "+temporary_root+"/"+targetName+".seq > "+temporary_root+"/"+targetName+".acc_";
	system(nnn_command.c_str());
	nnn_command="";
