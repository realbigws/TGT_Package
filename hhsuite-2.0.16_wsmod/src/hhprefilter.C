...
void init_prefilter()
{
  ...
  X = (unsigned char*)memalign(64,LDB*sizeof(unsigned char));                     // database string (concatenate all DB-seqs)
  first = (unsigned char**)memalign(64,(par.dbsize+2)*sizeof(unsigned char*));    // first characters of db sequences. Was (par.dbsize*2). Why??
  length = (int*)memalign(64,(par.dbsize+2)*sizeof(int));                         // lengths of db sequences Was (par.dbsize*2). Why??
