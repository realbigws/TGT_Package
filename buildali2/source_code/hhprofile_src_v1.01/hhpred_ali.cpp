...
// Object constructor
/////////////////////////////////////////////////////////////////////////////////////
Alignment::Alignment(int maxseq, int maxres, int par_matrix, int v_)
{
  v=v_;
  ...
  //substitution matrix
  SetSubstitutionMatrix(par_matrix);
}
...
{
  // copy substitution matrix
  for(int k=0;k<400;k++)
  {
    P[k]=ali.P[k];
    S[k]=ali.S[k];
  }
  for(int k=0;k<NAA;k++)pb[k]=ali.pb[k];
...
/////////////////////////////////////////////////////////////////////////////////////
void Alignment::Read(FILE* inf, char infile[], 
	int par_nseqdis, char* firstline, int par_mark)
{
...
//                  cerr<<endl<<"Error in "<<par.argv[0]<<": sequence "<<sname[k]<<" contains no residues."<<endl;
	  ...
	  cur_seq[l]='\0'; // avoids taking wrong sequence in case the input alignment is corrupted (two header lines with no sequence line between)
		...
		{display[k]=keep[k]=2; n_display++; kfirst=k;}
          ...
          //store all sequences
          else if (par_mark==0)                 {display[k]=keep[k]=1; n_display++;}
          //store sequences up to nseqdis
          else if (line[1]=='@'&& n_display-N_ss<par_nseqdis) {display[k]=keep[k]=2; n_display++;}
          else if (par_mark==1)                 {display[k]=keep[k]=1; n_display++;}
          //store sequences up to nseqdis
...
//        strcut_(ptr2);       // cut after first word ...
      ...
      //line contains sequence residues or SS information and does not belong to a >aa_ sequence
      else if (!skip_sequence)
        {
          ...
          if (v && l>=MAXCOL-1)
            {
              cerr<<endl<<"WARNING: maximum number of residues "<<MAXCOL-2<<" exceeded in sequence "<<sname[k]<<"\n";
              skip_sequence=1;
            }
...
//    	cerr<<endl<<"Error in "<<par.argv[0]<<": no sequences found in file "<<infile<<"\n"; 
  ...
  if (kfirst<0 || (N_in - N_ss -(keep[kfirst]==0? 1:0))==0 ) {
    fprintf(stderr, "Error : MSA file %s contains no master sequence!\n",infile); exit(1);
  }
...
/////////////////////////////////////////////////////////////////////////////////////
void Alignment::Compress(const char infile[],int par_M,int par_Mgaps)
{
  ...
  if (v>=3)
    {
      ...
      else if (par_M==2) cout<<"Using percentage-rule match state assignment\n";
      else if (par_M==3) cout<<"Using residues of first sequence as match states\n";
    }

  // Warn, if there are gaps in a single sequence
  if (v>=1 && N_in-N_ss==1 && par_M!= 2 && strchr(seq[kfirst]+1,'-')!=NULL)
  ...
  if (par_M==1) 
	...
	{
	  if (N_in-N_ss<=1) {
	    par_M=3; // if only single sequence in input file, use par.M=3 (match states by first seq)
...
//          cerr<<"\nError in "<<par.argv[0]<<": Alignment in "<<infile<<" contains no match states. Consider using -M first or -M <int> option"<<endl;
	...
	for (l=1; l<=L; l++) // for all positions l in alignment
	  {
	    ...
	    for (a=0; a<NAA; ++a) nl[a]=0;
	    for (k=0; k<N_in; ++k) if (keep[k]) nl[ (int)X[k][l]]++;
	    for (a=0; a<NAA; ++a) if(nl[a]) ++naa;
	...
	for (l=1; l<=L; l++)
	  {
	    if (percent_gaps[l]<=float(par_Mgaps))
	      {
		if (i>=MAXRES-2) {
		  if (v>=1)
		    fprintf(stderr,"WARNING: Number of match columns too large. Only first %i match columns will be kept!\n",i);
...
/////////////////////////////////////////////////////////////////////////////////////
int Alignment::FilterForDisplay(int max_seqid, int coverage, int qid, float qsc, int N, int par_mark)
{
  if (par_mark) return n_display;
...
// Remove sequences with seq. identity larger than seqid percent (remove the shorter of two) or coverage<cov_thr
/////////////////////////////////////////////////////////////////////////////////////
int Alignment::Filter(int max_seqid, int coverage, int qid, float qsc, int N)
{
              ...
              if (X[k][i]<NAA)
                {
                  ...
                  if (X[kfirst][i]<NAA)
                    {
                      gapq=0;
                      qsc_sum += S[(int)X[kfirst][i]*NAA+(int)X[k][i]];
                    }
                  else if (gapq++) qsc_sum-=PLTY_GAPEXTD; else qsc_sum-=PLTY_GAPOPEN;
                }
              else if (X[kfirst][i]<NAA)
                {
                  gapq=0;
      ...
      if (qdiff_max_frac<0.999)
        {
          ...
          for (int i=first[k]; i<=last[k]; ++i)
            ...
            if (X[k][i]<NAA && X[k][i]!=X[kfirst][i] && ++diff>=qdiff_max) break;
          //      printf("  diff=%4i\n",diff);
...
/////////////////////////////////////////////////////////////////////////////////////
void Alignment::FilterNeff(float par_Neff,int coverage,int max_seqid)
{
  int v1=v;
  v=v1-1;
  const float TOLX=0.01; 
  const float TOLY=0.02; 
  char *keep_orig = new char[N_in+1];   
  ...
  y=y0=filter_by_qsc(x0,keep_orig,coverage,max_seqid);
  if (y0<par_Neff) 
    {
      v=v1;
      if (v>=2) printf("Diversity of unfiltered alignment %.2f is below target diversity %.2f.\n",y0,par_Neff);
      delete [] keep_orig;
      return;
    }
  y1=filter_by_qsc(x1,keep_orig,coverage,max_seqid);
  
  while (fabs(par_Neff-y)>TOLY && x1-x0>TOLX)
    {
      x = x0 + (par_Neff-y0)*(x1-x0)/(y1-y0); // linear interpolation between (x0,y0) and (x1,y1)
      y = filter_by_qsc(x,keep_orig,coverage,max_seqid);
      if (v>=2) printf(" %3i  x0=%6.3f -> %6.3f     x=%6.3f -> %6.3f     x1=%6.3f -> %6.3f \n",++i,x0,y0,x,y,x1,y1);
      if (y>par_Neff) {x0=x; y0=y;} else {x1=x; y1=y;}
    }
  v=v1;
...
float Alignment::filter_by_qsc(float qsc, char* keep_orig,int coverage,int max_seqid)
{
  HMM q;
  for (int k=0; k<N_in; ++k) keep[k]=keep_orig[k];
  Filter2(keep,coverage,0,qsc,max_seqid+1,max_seqid,0); 
...
//(char keep[], int coverage, int qid, float qsc, int seqid1, int seqid2, int Ndiff)

/////////////////////////////////////////////////////////////////////////////////////
// Calculate AA frequencies q->f[i][a] and transition probabilities q->tr[i][a] from alignment
/////////////////////////////////////////////////////////////////////////////////////
void Alignment::FrequenciesAndTransitions(HMM* q, char* in, int par_wg)
{
  ...
  //---- assign background frequencies ------// 
  // [note]: from original HHsuite, the pb[20] is defined in hhdecl.C and calculated in hhmatrices.C
  for (a=0; a<NAA; ++a) q->pb[a]=pb[a];

  //---- copy master sequence (i.e., the first real sequence)
  // [note]: according to HHsuite, the first symbol of all sequences is ' ')
  int len=0;
  for(a=0;a<(int)strlen(seq[kfirst]); a++)
  {
    if(seq[kfirst][a]==' ')continue;
    if(seq[kfirst][a]>='a' && seq[kfirst][a]<='z')continue;
    q->seq[len]=seq[kfirst][a];
    len++;
  }
  q->seq[len]='\0';
  //final check
  if(len!=L)
  {
    fprintf(stderr,"match length not equal: %d != %d \n",len,L);
    exit(-1);
  }

  //---- copy sequence name
  strcpy(q->name,sname[kfirst]);
  ...
  if (N_filtered>1)
    {
      ...
      for (i=1; i<=L; ++i) // for all positions i in alignment
        {
          for (a=0; a<NAA; ++a) ni[a]=0;
          for (k=0; k<N_in; ++k) if (in[k]) ni[ (int)X[k][i]]++;
          naa=0; for (a=0; a<NAA; ++a) if(ni[a]) naa++;
          if (!naa) naa=1; //naa=0 when column consists of only gaps and Xs (=ANY)
          for (k=0; k<N_in; ++k)
            if (in[k] && X[k][i]<NAA)
      ...
      Amino_acid_frequencies_and_transitions_from_M_state(q,in,par_wg); // use subalignments of seqs with residue in i
      ...
      for (i=0; i<=L+1; ++i) // for all positions i in alignment
        {
          ...
          for (a=0; a<NAA; ++a) q->f[i][a]=0.0;
	  if (X[kfirst][i] < ANY)
	    q->f[i][(unsigned int) X[kfirst][i] ] = 1.0;
	  else
	    for (a=0; a<NAA; ++a) q->f[i][a]=pb[a];
          q->tr[i][M2M]=0;
          q->tr[i][M2I]=-100000.0;
          q->tr[i][M2D]=-100000.0;
          q->tr[i][I2M]=-100000.0;
  ...
  if (v>=3)
    {
      ...
      for (i=1; i<=L; ++i)
        {
          printf("%3i:  ",i);
          for (a=0; a<NAA; ++a)  printf("%4.0f ",100 * q->f[i][a]);
          cout<<endl;
        }
      cout<<"\n";
...
/////////////////////////////////////////////////////////////////////////////////////
void Alignment::Amino_acid_frequencies_and_transitions_from_M_state(HMM* q, char* in,int par_wg)
{
              ...
              for (j=1; j<=L; ++j)
                {
                  ...
                  naa=0; for (a=0; a<NAA; ++a) if(n[j][a]) naa++;
                  if (naa==0) continue;
                  ncol++;
                  for (k=0; k<N_in; ++k)
                    {
...
//       for (k=0; k<N_in; ++k) if (in[k]) w[k][i]=wi[k];
  ...
  for (a=0; a<NAA; ++a) q->f[0][a]=q->f[L+1][a]=pb[a];
                      ...
                      if (in[k] && I[k][i]>0 && X[k][j]<ANY)
                        {
                          if (!n[j][ (int)X[k][j]]) 
                          {
//                          	fprintf(stderr,"Error in %s: Ii=%i: n[%i][X[%i]]=0! (X[%i]=%i)\n",par.argv[0],i,j,k,k,X[k][j]);
                                fprintf(stderr,"Error : Ii=%i: n[%i][X[%i]]=0! (X[%i]=%i)\n",i,j,k,k,X[k][j]);
                          }
                          wi[k]+=1.0/float(n[j][ (int)X[k][j] ]*naa);
                        }
                    }
                      ...
                      if (in[k] && X[k][i]==GAP && X[k][j]<ANY)
                        {
                          if (!n[j][ (int)X[k][j]]) 
                          {
//                          	fprintf(stderr,"Error in %s: Di=%i: n[%i][X[%i]]=0! (X[%i]=%i)\n",par.argv[0],i,j,k,k,X[k][j]);
                                fprintf(stderr,"Error : Di=%i: n[%i][X[%i]]=0! (X[%i]=%i)\n",i,j,k,k,X[k][j]);
                          }
                          wi[k]+=1.0/float(n[j][ (int)X[k][j] ]*naa);
                        }
                    }
...
/////////////////////////////////////////////////////////////////////////////////////
void Alignment::WriteWithoutInsertsToFile(const char* alnfile,int par_append)
{
  if (v>=2) cout<<"Writing alignment to "<<alnfile<<"\n";
  FILE* alnf;
  if (!par_append) alnf = fopen(alnfile,"w"); else alnf = fopen(alnfile,"a");
...
/////////////////////////////////////////////////////////////////////////////////////
void Alignment::WriteToFile(const char* alnfile, const char format[],int par_append)
{
  FILE* alnf=NULL;
  ...
  for (j=hit.j1; j<=hit.j2; ++j)
    {
      ...
      while (hit.wwj[step]<j) step--;
      imatch[j] = hit.wwi[step];
  ...
  for (k=0; k<Tali.N_in; ++k)
    {
      ...
      if (N_in>=MAXSEQ)
        {
          fprintf(stderr,"WARNING : maximum number of %i sequences exceeded while reading %s. Skipping all following sequences of this MSA\n",MAXSEQ,ta3mfile);
          break;
        }
      ...
      if (j<hit.j1)
        {
//        	fprintf(stderr,"Error in %s: did not find %i match states in sequence %i of %s. Sequence:\n%s\n",par.argv[0],hit.j1,k,Tali.name,Tali.seq[k]); 
                fprintf(stderr,"Error: did not find %i match states in sequence %i of %s. Sequence:\n%s\n",hit.j1,k,Tali.name,Tali.seq[k]); 
        	exit(1);
        }
...
/////////////////////////////////////////////////////////////////////////////////////
void Alignment::GetPositionSpecificWeights(float* w[],int par_wg)
{
  // Calculate position-dependent weights wi[k] for each i.
  // For calculation of weights in column i use sub-alignment
  // over sequences which have a *residue* in column i (no gap, no end gap)
...
void Alignment::SetSubstitutionMatrix(int par_matrix)
{
  SetSubstitutionMatrix_ori(par_matrix,P,S,pb);
}
