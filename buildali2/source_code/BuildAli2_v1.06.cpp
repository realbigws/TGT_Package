...
void Usage() 
{
	...
	fprintf(stderr,"       [-m max_iter] [-e evalue] [-c cpu_num] [-r tmp_root] [-k remove] [-b] [-n] \n\n");
	...
	fprintf(stderr,"-e evalue :            E-value cutoff in BLAST. (default=0.001) \n\n");
...
int main(int argc,char **argv)
{
	//------ BuildAli2 -------//
	{
		...
		extern char* optarg;
		char c = 0;
		while ((c = getopt(argc, argv, "i:o:d:D:u:m:e:c:r:k:bn")) != EOF) {
			switch (c) {
			case 'i':
				query_file = optarg;
				break;
		...
		//process
		int cur_iter;
		cur_iter=BuildAli2_Main(max_iter,evalue,cpu_num,nr90,nr70,query_file,tmp_root,util_root,break_or_not);
